...
class TestMahalanobisDistance(unittest.TestCase):
    ...
    def test_mahalanobis_distance(self):
        ...
        m2 = mahalanobis_matrix(self.s1, self.s2, cov_matrix, rescale=RescalingTypeEnum.NONE)
        duration_2 = time.time() - start_time
        print(f"mahalanobis_matrix_2 execution time: {duration_2:.6f} seconds")
        
        # Timing mahalanobis_matrix_3
        start_time = time.time()
        m3 = mahalanobis_matrix(self.s1, self.s2, cov_matrix, rescale=RescalingTypeEnum.COVARIANCE)
