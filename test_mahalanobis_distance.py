...
class TestMahalanobisDistance(unittest.TestCase):
    ...
    def test_mahalanobis_distance(self):
        ...
        self.assertTrue(tf.reduce_all(tf.abs(m1 - m2) < self.epsilon),
                        msg=f"Max difference: {tf.reduce_max(tf.abs(m1 - m2))}")
        
        # Compare the distances within an epsilon range
        self.assertTrue(tf.reduce_all(tf.abs(m1 - m3) < self.epsilon),
                        msg=f"Max difference: {tf.reduce_max(tf.abs(m1 - m3))}")
        
        # Compare the distances within an epsilon range
        self.assertTrue(tf.reduce_all(tf.abs(m2 - m3) < self.epsilon),
                        msg=f"Max difference: {tf.reduce_max(tf.abs(m1 - m4))}")
