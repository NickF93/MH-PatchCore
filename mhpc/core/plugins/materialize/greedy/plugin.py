"""`greedy` materialization plugin implementation."""
...
class GreedyMaterializationPlugin(MaterializationPlugin):
    """Materialization plugin for `greedy`."""
        ...
        *,
        ...
        if unknown_keys:
            raise ValueError(
                ...
                "materialize plugin 'greedy'; unsupported keys: "
                f"{', '.join(unknown_keys)}"
            )
        ...
        if not isinstance(raw_compute_self_distances, bool):
            raise TypeError(
                "materialize plugin 'greedy' requires boolean "
