"""Plugin-local parameter binding/parsing for `feature_agg:ipca`."""
...
def _parse_feature_agg_params(params: Mapping[str, Any]) -> FeatureAggParams:
    ...
    pca_variance_ratio = float(params.get("pca_variance_ratio", 0.99))
...
class FeatureAggParamBindingMixin:
    """Feature-aggregation param parsing localized to `ipca`."""
    ...
    def resolve_reduction_selection(
        ...
        *,
        ...
        params = self._feature_agg_params
        return ReductionSelection(
            algorithm=algorithm,
            pca_variance_ratio=params.pca_variance_ratio,
        )
