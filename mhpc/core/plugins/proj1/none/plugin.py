"""No-op projector-1 plugin implementation."""
...
class NoneProjector1Plugin(Projector1Plugin):
    """Projector-1 plugin id `none` preserving identity behavior."""
    ...
    _bound_bind_context: Projector1BindContextLike
    ...
    def bind_params(
        ...
        *,
        ...
        if unknown_keys:
            raise ValueError(
                "pipeline.slots.proj1.params must be empty for "
                "proj1 plugin 'none'; unsupported keys: "
                f"{', '.join(unknown_keys)}"
            )
    ...
    def state_load(
        ...
        *,
        ...
        if state is not None:
            raise ValueError("proj1 plugin 'none' does not accept fitted state.")
