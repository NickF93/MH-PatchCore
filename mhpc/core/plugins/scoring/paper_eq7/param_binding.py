"""Plugin-local parameter binding/parsing for `paper_eq7` scoring."""
...
class ScoringParamBindingMixin:
    ...
    def bind_params(
        ...
        *,
        ...
        if patch_scoring_mode == "GLOBAL_TOPP_MEAN":
            disallowed_pni_keys = sorted(
                str(key)
        ...
        if patch_scoring_mode not in {"GLOBAL_TOPK_SOFT", "GLOBAL_TOPP_MEAN"}:
